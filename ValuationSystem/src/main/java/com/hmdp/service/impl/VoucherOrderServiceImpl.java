...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    // 创建阻塞队列，后面改为使用 Redis 的 Stream 消息队列
    ...
    private void handlePendingList() {
        try {
            while (true) {
                // 获取消息队列中的消息 XREADGROUP GROUP g1 c1 COUNT 1 STRAMS stream.orders 0
                List<MapRecord<String,Object,Object>> list = stringRedisTemplate.opsForStream().read(
                        Consumer.from("g1","c1"), // 组名和消费者名
    ...
    private void handleVoucherOrder(VoucherOrder voucherOrder) {
        ...
        /* 这里因为需要调用 createVoucher() 方法，所以不需要直接返回错误信息，而是到该函数中返回 */
    ...
    public void createVoucherOrder(VoucherOrder voucherOrder) {
        ...
        boolean success = seckillVoucherService.update()
                ...
                //  这里用 stock 当作版本号, 相当于实现了乐观锁, 只需要库存大于 0 即可以修改
                .gt("stock",0)
                .update();
        if(!success) {
            log.error("库存不足");
